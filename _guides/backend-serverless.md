...
> 适用对象：静态网站已经上线，现在需要一个「不能让访客看到密钥」的接口的人  
> 前置知识：同站的[《域名、网站上线与 Ubuntu 服务器指南》](/guides/domain-hosting/)（域名、DNS、HTTPS、宝塔与 Docker 部分）  
> 当前实例：`wangyuyue.xyz` 的[手机助手](/chat/)：网页留在 GitHub Pages，接口挂在 Cloudflare Worker 上  
> 更新日期：2026-09-22

前一篇讲的是「把网页发出去」，这一篇讲它的下一步：**网页还是静态的，但需要一个能保密的后端。**
...
- **来源白名单**：只回显自己站点的 `Origin`，并带上 `Vary: Origin`，别写成 `*`；
- **共享口令**：让前端多带一个自定义请求头（例如 `x-family-code`），后端逐个比对，不对就返回 401。

口令本身仍会随网页下发，只能挡住"随便看到地址就调用"的人，不能当身份认证用。真正要区分用户时，需要平台自带的鉴权。
...
上面 6 步里的第 3、4、6 步最容易被忽略：预检不通浏览器就直接拦掉；没有访问控制等于把额度挂在网上；流式不通则"逐字显示"会变成"等半天一次性出现"。

## 8. 常见坑与排错

### 8.1 npm 报 "node 不是内部或外部命令"
...
### 8.2 PowerShell 把 UTF-8 文件显示成乱码
...
这一篇讲的是「把后端挂上去」。挂上去之后，一次改动怎样才算真的到用户手里、出问题该从哪一层退回去，见[《发布与回滚：一次改动怎样才算真的上线》](/guides/publish-and-rollback/)。
